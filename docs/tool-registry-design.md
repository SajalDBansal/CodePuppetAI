...
### Step 5 — `apps/cli`: the client-side tool executor (depends on Stage 2 of the CLI build-out already having SSE consumption working)

- Add `@workspace/tool-registry` and `@workspace/protocol` as CLI dependencies (neither is one today).
- Resolve `workspaceRoot` from the CLI's existing config (`config.json`'s workspace-root concept, `packages/harness/src/store/config-store.ts`) once, at the start of a turn.
- Implement `promptUser` using the CLI's existing `inquirer` dependency (already installed, already used for other interactive prompts in `commands/account.ts`/`auth.ts`/etc.) — no new dependency needed for this.
- On a `tool_call` SSE event: look up the tool by name, check `requiresConfirmation` and prompt if needed (§6.3), call `execute(args, { workspaceRoot, promptUser })` for every category except `backend`, collect the result, and forward `backend`-category calls through unresolved as part of the next `toolResults` POST (the API resolves those, per §3/§4.6 — though in practice `backend` tools won't be registered client-side at all until `backend.ts` actually ships).
- Wrap each `execute()` call in try/catch at the executor level too, as a second line of defense on top of §6.6's per-tool contract — a tool implementation bug still shouldn't be able to kill the CLI process mid-turn.
